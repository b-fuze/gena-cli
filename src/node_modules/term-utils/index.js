...
exports.ansiRegex = /\x1B[\x40-\x5F]([\x30-\x3F]*[\x20-\x2F])*[\x30-\x3F]*[\x40-\x7E]/g;
function stripAnsi(string) {
    return string && string.replace(exports.ansiRegex, "");
}
exports.stripAnsi = stripAnsi;
var ResetSlice;
(function (ResetSlice) {
    ResetSlice[ResetSlice["NONE"] = 0] = "NONE";
    ResetSlice[ResetSlice["ALL"] = 63] = "ALL";
    ResetSlice[ResetSlice["COLOR"] = 1] = "COLOR";
    ResetSlice[ResetSlice["BG"] = 2] = "BG";
    ResetSlice[ResetSlice["BOLD"] = 4] = "BOLD";
    ResetSlice[ResetSlice["DIM"] = 4] = "DIM";
    ResetSlice[ResetSlice["ITALIC"] = 8] = "ITALIC";
    ResetSlice[ResetSlice["UNDERLINE"] = 16] = "UNDERLINE";
    ResetSlice[ResetSlice["STRIKETHRU"] = 32] = "STRIKETHRU";
})(ResetSlice = exports.ResetSlice || (exports.ResetSlice = {}));
function resetAnsi(options) {
    // FIXME: Maybe remove switch
    switch (options) {
        case 0: return "";
        case 63: return "\x1b[39m\x1b[49m\x1b[22m\x1b[23m\x1b[24m\x1b[29m";
    }
    let reset = "";
    ResetSlice.COLOR & options && (reset += "\x1b[39m");
    ResetSlice.BG & options && (reset += "\x1b[49m");
    ResetSlice.BOLD & options && (reset += "\x1b[22m");
    // Skip DIM
    ResetSlice.ITALIC & options && (reset += "\x1b[23m");
    ResetSlice.UNDERLINE & options && (reset += "\x1b[24m");
    ResetSlice.STRIKETHRU & options && (reset += "\x1b[29m");
    return reset;
}
function sliceAnsi(string, start = 0, end = undefined, reset = ResetSlice.NONE, meta = false) {
    let resetCode = "";
    switch (reset) {
        case 0: break;
        case 63:
            resetCode = "\x1b[39m\x1b[49m\x1b[22m\x1b[23m\x1b[24m\x1b[29m";
            break;
        default: resetCode = resetAnsi(reset);
    }
    // Empty string
    if (string.length === 0) {
        // Nothing to return, just reset code
        return meta ? { out: resetCode, length: 0, oldLength: 0 } : resetCode;
    }
    const codes = [
        ["", 0, 0],
    ];
    exports.ansiRegex.lastIndex = 0;
    ...
    codeSearchLoop: while ((codeIndex = string.indexOf("\u001b", codeSearchStart)) !== -1) {
        let codeString = "\u001b";
    ...
    if (!undefinedEnd && end < start || end === start) {
        // Invalid range
        return meta ? { out: resetCode, length: 0, oldLength: rawStringLength } : resetCode;
    }
    // Find where start/end are relative to the escape codes
    // and find endTextCode if any
    if (remapIndexes) {
    ...
    if (codes.length === 1) {
        ...
        const sliced = string.slice(start, end) + resetCode;
    ...
    const fullOut = outInitialCodes + out + resetCode;
    return meta ? { out: fullOut, length: outLength, oldLength: rawStringLength } : fullOut;
}
exports.sliceAnsi = sliceAnsi;
function forceAnsi(string, ansi) {
    return string.replace(/\u001b\[0m/g, "\u001b[0m" + ansi);
}
exports.forceAnsi = forceAnsi;
function rightAlign(content, precedence = false, slideUnprecedented = false) {
    return "\\]" + (precedence ? "R" : "r") + (slideUnprecedented ? "/" : "\\") + content;
}
exports.rightAlign = rightAlign;
function center(content, cols) {
    const printable = stripAnsi(content).length;
    const space = Math.max(cols - printable, 0) / 2;
    return jshorts_1.jSh.nChars(" ", Math.floor(space))
        + content
        + jshorts_1.jSh.nChars(" ", Math.ceil(space));
}
exports.center = center;
function strFill(string, cols) {
    let out = string;
...
function strMultiply(string, count) {
    if (count <= 0) {
        return "";
    }
    let out = "";
...
exports.strMultiply = strMultiply;
