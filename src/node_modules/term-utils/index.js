...
function sliceAnsi(string, start = 0, end = undefined, reset = false, verbose = true) {
    // Empty string
    if (string.length === 0) {
        return "";
    }
    ...
    codeSearchLoop: while (~(codeIndex = string.indexOf("\u001b", codeSearchStart))) {
        let codeString = "\u001b";
        ...
        for (let i = codeIndex + 2; i < string.length; i++) {
            ...
            else if (!(curCode >= 0x30 && curCode <= 0x3F || curCode >= 0x20 && curCode <= 0x2F)) {
                codeSearchStart = i;
                continue codeSearchLoop;
            }
            codeString += cur;
    ...
    if (codes.length === 1) {
        ...
        if (beforeStart) {
            ...
            let suffix;
            out += middleText;
            out += suffix = string.slice(suffixTextIndex, endTextCodeItem ? Math.min(endTextCodeItem[1], suffixEnd) : suffixEnd);
        }
    }
...
function rightAlign(content, precedence = false) {
