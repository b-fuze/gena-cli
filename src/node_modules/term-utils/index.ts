...
export enum ResetSlice {
  NONE       = 0,
  ALL        = 63,
  COLOR      = 1,
  BG         = 2,
  BOLD       = 4, // Bold and dim share the same close codes
  DIM        = 4,
  ITALIC     = 8,
  UNDERLINE  = 16,
  STRIKETHRU = 32,
...
function resetAnsi(options: ResetSlice): string {
...
export function sliceAnsi(string: string, start?: number, end?: number, reset?: ResetSlice, meta?: false): string;
export function sliceAnsi(string: string, start?: number, end?: number, reset?: ResetSlice, meta?: true): SliceMeta;
export function sliceAnsi(string: string, start: number = 0, end: number = undefined, reset = ResetSlice.NONE, meta = false) {
  let resetCode = "";

  switch (reset) {
    case 0:  break;
    case 63: resetCode = "\x1b[39m\x1b[49m\x1b[22m\x1b[23m\x1b[24m\x1b[29m"; break;
...
export function rightAlign(content: string, precedence = false, slideUnprecedented = false) {
